...
import { 
  ...
  FileText, AlertCircle, Loader2, Sparkles, Download, Copy
} from "lucide-react";
...
export default function CustodyPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const prompt = `Generate a comprehensive child custody plan with the following details:
- Parent 1: ${formData.parentName}
- Parent 2: ${formData.otherParentName}
- State: ${formData.state}
- Children: ${formData.childrenNames} (${formData.childrenCount} children)
- Current Arrangement: ${formData.currentCustody}
- Desired Arrangement: ${formData.desiredCustody}
- Special Concerns: ${formData.concerns || "None specified"}

Create a detailed, professional custody plan that includes:
1. Custody arrangement type and legal framework for ${formData.state}
2. Detailed parenting schedule (weekly, holidays, summers, special occasions)
3. Decision-making authority for education, healthcare, and activities
4. Communication guidelines between parents and with children
5. Transportation and exchange logistics
6. Modification procedures
7. Dispute resolution process

Format as a professional legal document. Include ${formData.state}-specific legal requirements.`;
      ...
      toast.success("Custody plan generated by CustodiAI!");
    } catch (error: any) {
      toast.error(error.message || "Failed to generate plan");
      // Fallback to basic plan if AI fails
      setGeneratedPlan(`
CHILD CUSTODY PLAN - PREVIEW
...
This is a preview. Full AI-generated plan with detailed schedules and state-specific provisions available with premium access.

⚠️ DISCLAIMER: This is for informational purposes only. Consult a licensed family law attorney.
      `.trim());
    } finally {
